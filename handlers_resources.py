...
async def list_orders(params: ListOrderParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({"orders": items, "total": len(items)}, summary=f"Found {len(items)} orders.")
...
async def get_order(params: GetOrderParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({"id": rid, "name": rname, "status": r.get("status"), "created_at": r.get("createdAt") or r.get("created_at"), "raw": r}, summary=f"Retrieved Order {rid}.")
    except Exception as e:
