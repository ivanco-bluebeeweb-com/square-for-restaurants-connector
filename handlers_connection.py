"""Connection lifecycle for Square for Restaurants Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from square_restaurants_client import SquareforRestaurantsClient
...
_SECRET = "square_restaurants_connections"
...
@chat.function(
    "connect_square_restaurants",
    "Connect Square for Restaurants account via credentials.",
    action_type="write",
    chain_callable=True,
    event="square-for-restaurants-connector.connect_square_restaurants",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_square_restaurants(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Square for Restaurants Connector."""
    client = SquareforRestaurantsClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Square for Restaurants Account",
...
@chat.function(
    "list_connections",
    "List connected Square for Restaurants accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_square_restaurants",
    "Disconnect Square for Restaurants account.",
    action_type="write",
    chain_callable=True,
    event="square-for-restaurants-connector.disconnect_square_restaurants",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_square_restaurants(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
